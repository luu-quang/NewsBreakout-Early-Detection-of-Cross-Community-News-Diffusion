...
def analyze(df: pd.DataFrame) -> dict:
    ...
    # Stage 1: exact duplicates = identical normalized title AND identical normalized description
    # (two empty descriptions count as identical text).
    groups: dict[tuple, list[int]] = defaultdict(list)
    for f in feats:
        groups[(f["title"], f["desc"])].append(f["pos"])
    edges: dict[tuple[int, int], tuple[str, dict, str]] = {}
    for members in groups.values():
        if len(members) > 1 and feats[members[0]]["title"]:
            for i, j in combinations(sorted(members), 2):
                edges[(i, j)] = ("EXACT_DUPLICATE", {"title_sim": 1.0, "desc_sim": 1.0, "number_sim": None, "hours_apart": None}, "identical_normalized_title_and_description")
...
def main() -> None:
    ...
    out = df.copy()
    out["duplicate_family_id"] = pd.Series(result["assignment"], dtype="object")
