...
python -m unittest test_dedup_independent -v     # from independent_impl/
```
...
The summary records the current Git revision. A `+uncommitted-changes` suffix means the run was
made from a working tree containing changes, including regenerated output artifacts.

## Method

Phase 2B uses a precision-first approach. Exact URL/text matches identify exact duplicates.
Likely syndicated copies require high description overlap, temporal proximity, and numeric
consistency. Articles that appear to describe the same event but lack copy-level evidence are
intentionally preserved as independent reports for Phase 2C event clustering.
...
SAME_EVENT_INDEPENDENT is a QC / Phase 2C handoff label only and does not contribute edges to duplicate families.
...
gap. The 0.60 threshold is a conservative heuristic selected from this small manual sample,
not a universal cutoff. It is a placeholder rather than a validated operating point and favors
...
The corpus contains **no exact duplicates under either final exact-match rule**, including the
new normalized canonical-URL rule. The two families are a
